...
UNIQ_SENTENCE_FILE="${CACHE_DIR}/train_uniq.txt"
...
if [ "${LOWER_CASE}" = "True" ]; then
    if [ "${MOST_LOW}" = "True" ]; then
        LOWERCASE_SENTENCE_FILE="${CACHE_DIR}/train_mostlow.txt"
...
if [ ! -f "${SENTENCEPIECE_MODEL_NAME}.model" ]; then
  eval spm_train '--input="${INPUT_SENTENCE_FILE}"'\
            ...
            --model_type=unigram\
            --num_threads=10\
            "${USER_DEF_SYMBOLS:+--user_defined_symbols='$USER_DEF_SYMBOLS'}" 2>&1 | grep -v "LOG(WARNING) Empty string found. removed"
else
